...
import {
  ...
  TableHead, TableRow, LinearProgress, Chip
} from '@mui/material';
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import AddCircleOutlineIcon from '@mui/icons-material/AddCircleOutline';
import DeleteIcon from '@mui/icons-material/Delete';
import { addHarvestToProperty, deleteHarvest, addCropToHarvest, deleteCrop, type Harvest } from '../../stores/producer/slice';
...
const PropertyDetail: React.FC = () => {
  ...
  const farm = useSelector((state: RootState) =>
    state.producerReducer.producers.flatMap(p => p.farms).find(f => f.id === propertyId)
  );

  if (!farm) {
    return <Typography>Propriedade não encontrada.</Typography>;
  }

  const totalHarvestArea = farm.harvests.reduce((sum, h) => sum + h.total_area_ha, 0);
    ...
    <Container maxWidth="lg">
      ...
      <Paper sx={{ p: 3, mt: 2 }}>
        ...
        <Grid container spacing={2} sx={{ mt: 2 }}>
          ...
          <Grid size={{ xs: 4 }}><Chip label={`Área de Vegetação: ${farm.vegetation_area_ha} ha`} color="warning" sx={{ width: '100%' }} /></Grid>
        </Grid>
